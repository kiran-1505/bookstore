    ...
    public function getAvailableBooks ()
    {
        try {
            $this->stat = $this->conn->prepare("
             SELECT a.name as author, b.book_id,b.title,b.sell_price,b.ISBN,b.buy_price,p.name as semester_name,p.semester_id,b.quantity,b.actual_quantity,b.description,b.image
             FROM author a JOIN book b ON a.author_id=b.author_id 
                 JOIN semester p on b.semester_id = p.semester_id
             WHERE quantity>0 ");
            $this->stat->execute();
            while ($row = $this->stat->fetch(PDO::FETCH_ASSOC)) {
                $semester = new Semesters($row['semester_name'], $row['semester_id']);
                new Book(
                    $row['book_id'],
                    $row['title'],
                    $row['Course Code'],
...
WHERE a.name like ? AND  b.quantity>0;");
    ...
    public function getBookInfo ($id): Book
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.book_id,b.title,b.ISBN,b.sell_price,b.buy_price,b.image,b.quantity,b.actual_quantity,b.description,a.name as author,p.name as semester_name
FROM book b,author a,publisher p
WHERE b.book_id=?
AND b.author_id=a.author_id
AND p.semester_id=b.semester_id;
");
            ...
            return new Book(
                ...
                $row['quantity'],
                new Author($row['author'], ''),
                new Semesters($row['semester_name'])
            );
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            die();
    ...
    public function getNotAvailableBooks ()
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.book_id,b.title,b.sell_price,b.buy_price,b.description,b.actual_quantity,p.phone as semester,p.phone as semester_phone,sum(od.quantity) as co
from book b,order_details od,orders o,semester p
where o.order_date <= TIMESTAMPADD(YEAR ,1,o.order_date)
...
ORDER BY sum(od.quantity) DESC ;");
    ...
    public function getBooksOffSemester (int $semid): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT b.book_id,b.title,b.image,b.sell_price,b.buy_price,b.ISBN
            FROM publisher p JOIN book b on p.semester_id = b.semester_id
            WHERE p.semester_id=? AND b.quantity>0;");
    ...
    public function cashBooks (int $search, string $str): void
    {
        ...
        try {
            ...
            while ($row = $this->stat->fetch(PDO::FETCH_ASSOC))
                new Book(
                    $row['book_id'], $row['title'],
                    $row['Course Code'], $row['sell_price'],
                    $row['buy_price'], $row['image'],
...
/// they do not test
    ...
    public function getAllsemesters (): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT publisher_id,name FROM semester ORDER BY  2 ");
            $this->stat->execute();
            return $this->stat;
        } catch (PDOException $exception) {
            echo $exception->getMessage();
    ...
    public function getSemester ($id): Semesters
    {
        try {
            $this->stat = $this->conn->prepare("
            ...
            $this->stat->execute();
            $row = $this->stat->fetch(PDO::FETCH_ASSOC);
            return new Semesters($row['name'], $row['semester_id']);
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            return NULL;
        }
    ...
    public function getSemessterbyname ($name): Semesters
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT semester_id,name FROM semester where name=?;");
