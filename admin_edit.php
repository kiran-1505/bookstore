<?php
	...
	// get book data
	$query = "SELECT * FROM books WHERE course_code = '$course_code'";
...
?>
	<form method="post" action="edit_book.php" enctype="multipart/form-data">
		<table class="table">
			<tr>
				<th>Course Code</th>
				<td><input type="text" name="isbn" value="<?php echo $row['course_code'];?>" readOnly="true"></td>
			</tr>
			<tr>
				<th>Title</th>
			...
			<tr>
				<th>Semester</th>
				<td><input type="text" name="semester" value="<?php echo getSemName($conn, $row['semesterid']); ?>" required></td>
			</tr>
		</table>
