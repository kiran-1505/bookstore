...
class Take extends Talking
{
    ...
    public function takeBook(Book $book):bool {
        try{
            $this->stat=$this->conn->prepare("
            insert into book(book_id, title, author_id, semester_id, genre_id, ISBN, sell_price, buy_price, image, description, quantity, actual_quantity) 
            values (?,?,?,?,?,?,?,?,?,?,?,?);");
            ...
            $this->stat->bindValue(4,$book->getSemester()->getId(),PDO::PARAM_INT);
    ...
    public function updateBook(Book $book):bool{
        try{
            ...
            $this->stat=$this->conn->prepare("
            UPDATE book SET author_id=?,semester_id=?,buy_price=?,sell_price=?,quantity=?,
            actual_quantity=?,image=? WHERE book_id=?;  
            ");
            $this->stat->bindValue(1,$book->getAuthor()->getId(),PDO::PARAM_INT);
            $this->stat->bindValue(2,$book->getSemester()->getId(),PDO::PARAM_INT);
