<?php
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT course_code, book_image FROM books ORDER BY course_code DESC";
	...
	function getBookByIsbn($conn, $isbn){
		$query = "SELECT book_title, book_author, book_price FROM books WHERE course_code = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getbookprice($isbn){
		$conn = db_connect();
		$query = "SELECT book_price FROM books WHERE course_code = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getSemName($conn, $semid){
		$query = "SELECT semester_name FROM semester WHERE semesterid = '$semid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getAll($conn){
		$query = "SELECT * from books ORDER BY course_code DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
